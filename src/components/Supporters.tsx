...
export const Supporters: React.FC = () => {
  return (
    <section id="support" className="py-24 lg:py-32 bg-slate-950 text-white relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-12 lg:gap-20 items-start">
          <div>
            ...
            <h2 className="mt-4 text-4xl sm:text-5xl font-medium tracking-[-0.04em] leading-[1.08]">
              Conversas melhores.<br />Com gente por trás.
            </h2>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-8 inline-flex items-center gap-2.5 px-6 py-3.5 rounded-full bg-white text-slate-950 hover:bg-emerald-50 font-bold text-sm transition-colors cursor-pointer"
            >
          ...
          <div className="space-y-3">
            {SUPPORT_PATHS.map(({ icon: Icon, title, text }, index) => (
              <article key={title} className="group flex gap-5 p-5 sm:p-6 rounded-2xl border border-white/10 bg-white/[0.035] hover:bg-white/[0.06] hover:border-emerald-400/30 transition-colors">
                <span className="shrink-0 w-11 h-11 rounded-xl bg-emerald-400/10 text-emerald-400 flex items-center justify-center">
        ...
        <div className="relative mt-16 pt-8 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <p className="text-sm text-slate-400">Apoiadores fundadores</p>
          <p className="text-sm text-slate-500">Este espaço será de quem ajudar o Parrot a chegar mais longe.</p>
        </div>
      </div>
    </section>
  );
